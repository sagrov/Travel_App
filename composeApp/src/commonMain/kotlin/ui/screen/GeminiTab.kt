...
fun GeminiScreenView(navigator: Navigator, viewModel: HomeScreenModel){
    ...
    if (navigateToGemini.first && navigateToGemini.second != null) {
        val customPrompt = """
                    Як турист, я хочу дослідити та дізнатися більше про місце призначення. Будь ласка, надайте всебічну інформацію про наступне місце: ${navigateToGemini.second?.title}.
                    Включіть ключові деталі, такі як:
                    - Короткий огляд місця
                    - Історична або культурна значущість
                    - Популярні туристичні атракції чи пам'ятки
                    - Найкращий час для відвідування
                    - Доступні активності
                    - Маршрути для навігації або як дістатися з популярних місць
                    Зробіть інформацію цікавою та легкою для розуміння.
                """.trimIndent()
