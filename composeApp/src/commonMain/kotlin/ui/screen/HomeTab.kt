...
import travelbuddy.composeapp.generated.resources.people
...
import ui.component.loadPeopleItem
...
fun HomeScreenView(
    ...
    Surface(modifier = Modifier.fillMaxWidth().padding(bottom = BOTTOM_NAV_SPACE)) {
        ...
        VerticalScrollLayout(
            ...
            ChildLayout(
                contentType = HomeScreenContents.PEOPLES_VIEW_ALL.name,
                content = {
                    TitleWithViewAllItem(
            ...
            ChildLayout(
                contentType = HomeScreenContents.PEOPLES_SECTION.name,
                content = {
                    loadPeopleItem(peoples) { people ->
                        when (people.title) {
                            "Всі" -> {
                                mDestinations = destinations
                                mNearestDestinations = FakeArticles.destinations
                                ...
                                mDestinations = arrayListOf<Destination>().apply {
                                    addAll(destinations.filter { it.people == people })
                                }
                                mNearestDestinations = arrayListOf<Destination>().apply {
                                    addAll(nearestDestinations.filter { it.people == people })
                                }
                            }
                        }
                    }
